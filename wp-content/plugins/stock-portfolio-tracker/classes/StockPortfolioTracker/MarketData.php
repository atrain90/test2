...
class MarketData
{
    ...
    public function get()
    {
        ...
        if (isset($this->config[$this->params['type']]) && is_array($this->params['assets']) && !empty($this->params['assets'])) {
            $assets = [];
            ...
            if (!empty($assets)) {
                ...
                foreach ($urls as $url) {
                    ...
                    $dataItems = Helper::getObjectProperty($response, $this->config[$this->params['type']]['result']);
                    if (!empty($dataItems)) {
    ...
    {
        $urls = [];
        $baseUrl = $this->config[$this->params['type']]['url'];
        ...
        } elseif ($this->params['type'] == 'quotes-extended') {
            foreach ($assets as $asset) {
    ...
    {
        return $this->params['type'] == 'history' ?
            $this->config[$this->params['type']]['cache'][$this->params['interval']] :
            $this->config[$this->params['type']]['cache'];
    }    
}