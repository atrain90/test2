...
class Activity_Main_API {
	...
	function __construct()
	{
		...
		add_filter( 'pre_kses', array( &$this, 'allow_get_params' ) );
	...
	function save( $array = array(), $update_post = FALSE, $update_post_id = NULL ) {
		...
		$_permalink = add_query_arg( 'wall_post', $post_id, get_permalink( UM()->permalinks()->core['activity'] ) );
	...
	{
		$number = UM()->options()->get( 'activity_posts_num' );
		...
		UM()->Activity_API()->shortcode()->args = $args;
	...
	function ajax_load_more_comments()
	{
		...
		$comments = get_comments( array( 'post_id' => $post_id, 'parent' => 0, 'number' => $number, 'offset' => $offset, 'order' => UM()->options()->get( 'activity_order_comment' ) ) );
	...
	function ajax_load_more_replies()
	{
		...
		$child = get_comments( array( 'post_id' => $post_id, 'parent' => $comment_id, 'number' => $number, 'offset' => $offset, 'order' => UM()->options()->get( 'activity_order_comment' ) ) );
		$child_all = get_comments( array( 'post_id' => $post_id, 'parent' => $comment_id, 'number' => 999, 'offset' => 0, 'order' => UM()->options()->get( 'activity_order_comment' ) ) );
		...
		if (!$output['error']) {
			...
			if ($_POST['_post_id'] == 0) {
				...
				if ($_post_img) {
					...
					$photo_uri = UM()->files()->new_user_upload( get_current_user_id(), $um_is_temp_image, '_um_wall_img_upload' );
			...
			// other output
			$output['permalink'] = add_query_arg( 'wall_post', $post_id, get_permalink( UM()->permalinks()->core['activity'] ) );
